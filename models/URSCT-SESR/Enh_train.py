...
from model.URSCT_model import URSCT, ZeroURSCT
from pytorch_msssim import MS_SSIM
from model.Gradient_Loss import Gradient_Loss
from model.Charbonnier_Loss import L1_Charbonnier_loss as Charbonnier_Loss
from model import ZeroLoss
...
def main(opt):
    ...
    model = ZeroURSCT(model_detail_opt).cuda()
    ...
    scheduler_cosine = optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        optim_opt['EPOCHS'] - warmup_epochs,
    ...
    L_color = ZeroLoss.L_color().cuda()
    L_spa = ZeroLoss.L_spa().cuda()
    L_exp = ZeroLoss.L_exp(16).cuda()
    L_TV = ZeroLoss.L_TV().cuda()
    ...
    train_loader = DataLoader(
        dataset=get_training_data(train_data_dir, {'patch_size': train_opt['TRAIN_PS']}),
        batch_size=optim_opt['BATCH'],
        shuffle=True,
        num_workers=train_opt['NUM_WORKS'])

    val_loader = DataLoader(
        dataset=get_validation_data(val_data_dir, {'patch_size': train_opt['VAL_PS']}),
        batch_size=1,
        shuffle=True,
        num_workers=train_opt['NUM_WORKS'])
    ...
    for epoch in range(start_epoch, optim_opt['EPOCHS'] + 1):
        ...
        epoch_loss_total, Loss_TV, loss_spa, loss_col, loss_exp = 0, 0, 0, 0, 0
        ...
        for i, data in enumerate(pbar):
            ...
            # best_loss
            loss_total = Loss_TV + loss_spa + loss_col + loss_exp
        ...
        if epoch % train_opt['VAL_INTERVAL'] == 0:
            ...
            save_image(
                torch.cat(
                    (TF.resize(input_[0], (train_opt['VAL_PS'][0], train_opt['VAL_PS'][1])),
                     restored_enh[0],
                     target_enh[0]),
                    -1),
        ...
        print("------------------------------------------------------------------")
        print(
            "[Epoch] {} [Time] {:.4f} [Loss] {:.4f}  [Learning Rate] {:.6f}".format(
                epoch,
