...
def train(config):
    ...
    for epoch in range(config.num_epochs):
        ...
        for iteration, img_lowlight in enumerate(train_loader):
            ...
            loss_cos = 1 - nn.functional.cosine_similarity(enhanced_image, img_ref, dim=1).mean() if config.cos_loss else torch.zeros([]).cuda()
            ...
            if iteration == 0:
                ...
                A = A.sub(A.view(n, _, -1).min(dim=-1)[0].view(n, _, 1, 1)).div(
                    A.view(n, _, -1).max(dim=-1)[0].view(n, _, 1, 1) - A.view(n, _, -1).min(dim=-1)[0].view(n, _, 1, 1)).squeeze()

                writer.add_image('input_enhanced_ref_residual',
                                 torch.cat([img_input[0],
                                            enhanced_image[0],
                                            img_ref[0],
                                            torch.abs(enhanced_image[0] - img_ref[0])] + [torch.stack((A[0, i], A[0, i], A[0, i]), 0) for i in range(A.shape[1])], 2 ),
                                 epoch)
                print("------------------Epoch %d--------------------" % epoch)
...
def get_args():
    ...
    parser.add_argument('--model', type=str, default='DCE-Net')
...
if __name__ == "__main__":
    ...
    config.lowlight_images_path = "./data/"
    config.parallel = False
    config.snapshots_folder = 'snapshots/STAR-ori'
    config.lr = 0.001
    config.num_epochs = 100
    config.lr_type = 'cos'
    config.train_batch_size = 8
    config.model = 'STAR-DCE-Ori'
    config.snapshot_iter = 10
    config.num_workers = 4

    # CNN-based DCE-Net
    config.snapshots_folder = 'snapshots/DCE'  # DCE-Net-Pool
    config.model = 'DCE-Net'
